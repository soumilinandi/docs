...
When you enable [persistence](/oss/langgraph/persistence), you can [inspect the graph state](/oss/langgraph/persistence#checkpoints) (checkpoint) via the appropriate method. To include subgraph state in the result, pass the subgraphs option. The returned `StateSnapshot` includes nested `StateSnapshot` objects accessible via `.tasks[<task_index>].state`.
...
A common use case is inspecting subgraph state when the subgraph is [interrupted](/oss/langgraph/interrupts). While the graph is paused, you can view the subgraph state, and then resume execution.
...
You can update subgraph state by passing the subgraph's state config to the update state method. This is useful for modifying subgraph state during [human-in-the-loop](/oss/langgraph/interrupts) workflows.
