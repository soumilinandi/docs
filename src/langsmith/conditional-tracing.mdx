...
## Customize tracing in deployed graphs

When using a [factory function](/langsmith/graph-rebuild) to rebuild your graph at runtime, you can wrap the yielded graph with `tracing_context` to control tracing per-execution. This is useful for disabling tracing entirely for a specific graph, or for customizing tracing based on the authenticated user.
...
async def make_graph(runtime: ServerRuntime):
    ...
    # Route traces to a per-tenant project, or disable tracing entirely
    if user and should_disable_tracing(user.identity):
        with ls.tracing_context(enabled=False):
            yield graph
    elif user:
        with ls.tracing_context(
            enabled=True,
            project_name=f"tenant-{user.identity}",
