...
var PreviewMessageSchema = z.union([
  HostToGuestMessageSchema,
  GuestToHostMessageSchema
]);

// ../preview-protocol/src/host.ts
function createPreviewHost(iframe, options) {
  const targetOrigins = options?.targetOrigins ?? ["*"];
...
`;
  ...
  const [sdk, setSdkRaw] = useState(() => sdkCache.get(sdkCacheKey) ?? defaultSdk);
  const setSdk = useCallback((s) => {
    sdkCache.set(sdkCacheKey, s);
    setSdkRaw(s);
  }, [sdkCacheKey]);
  const [sdkDropdownOpen, setSdkDropdownOpen] = useState(false);
  const previewUrl = agentServer === "prod" ? SDK_PROD_HOSTS[sdk] : SDK_LOCAL_HOSTS[sdk];
  ...
  useEffect(() => {
    ...
    if (!cached) {
      ...
      cached = { iframe: iframe2, host: host2, ready: false, lastHeight: minHeight, lastView: defaultView };
  ...
  }, [cacheKey, previewUrl, agentQuery, minHeight, maxHeight]);
  useEffect(() => {
  ...
  }, [sdkDropdownOpen, sdk, effectiveTheme]);
  ...
  return <div data-lc-pe className={`${effectiveTheme === "dark" ? "dark" : ""} ${className ?? ""}`}>
    ...
    /* Tab bar */
        ...
        {showCodeTab && <div className="flex items-center justify-between px-3 py-2 border-b lc-border lc-bg-wash">
            <div
    className="inline-flex items-center gap-0.5 rounded-lg border lc-border lc-bg-surface"
    style={{ padding: 3 }}
  >
              <button
    type="button"
  ...
  >
              ...
              {(traceUrl || traceLoading) && agentServer !== "local" && <a
    href={traceUrl ?? "#"}
    target="_blank"
  ...
  />}
    ...
    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border lc-sdk-btn font-medium cursor-pointer transition-colors"
  >
