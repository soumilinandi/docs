...
The [LangChain frontend patterns](/oss/python/langchain/frontend/overview) — markdown messages, tool calling, human-in-the-loop, and more — all work with deep agents too. Deep agents are built on the same LangGraph runtime, so `useStream` provides the same core API.
