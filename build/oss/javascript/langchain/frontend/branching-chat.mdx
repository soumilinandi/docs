...
| `firstSeenState.parent_checkpoint` | The checkpoint just before this message — used as the fork point for edits and regenerations |
...
Combine branching with [optimistic updates](/oss/javascript/langchain/frontend/optimistic-updates)
for a seamless editing experience. When the user saves an edit, optimistically
show the updated message before the server responds:
